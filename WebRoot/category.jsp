...
Category room_c = CategoryManager.getCategoryByName(category_name);
if(room_c == null){
	String id = request.getParameter("id");
	room_c = CategoryManager.getCategoryById(id);
}
if(room_c == null){
	%> <jsp:forward page="errorpage/errpage.jsp?mes=亲。没有找到此分类哦... "/> <%
return;
}
%>
...
Post p = null;
int i=0;
String category_id = String.valueOf(room_c.getId());
...
int post_num = PostManager.getCategoryPostNum(category_id);
int page_count = 1;
	...
	<div class="nav_main">
		<ul class="nav_ul">
			...
			while(nav_c_it.hasNext()){
				...
				if(room_c.getId() == nav_c.getId()) nav_id = "nav_active";
				else nav_id = "";
				%>
...
<div class="content">
	...
	<ul id="top_topic_ul">
		<%
		posts = PostManager.getPublishCategoryPostsByScore(0, 6,category_id);
			    	it = posts.iterator();
			    	p = null;
		for(i=0; i<6; i++) {
			if(it.hasNext()){
    ...
    <div class="posts">
    	<%
    		if(page_num != 0) {
    	    	    	    		posts = PostManager.getPublishCategoryPosts(post_info_num*(page_num-1), post_info_num,category_id);
    	    	    	    	}else {
    	    	    	    		posts = PostManager.getPublishCategoryPosts(0,post_info_num,category_id);
    	    	    	    	}
    	    	    	    	it = posts.iterator();
    	    	    	    	p = null;
