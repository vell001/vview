...
User author = UserManager.getUserByName(author_name);
if(author == null){
	author = UserManager.getUserById(request.getParameter("author_id"));
}
if(author == null){
	%> <jsp:forward page="errorpage/errpage.jsp?mes=亲。没有找到此用户空间哦... "/> <%
return;
}
%>
...
Post p = null;
int i=0;
String author_id = String.valueOf(author.getId());
...
int post_num = PostManager.getAuthorPostNum(author_id);
int page_count = 1;
...
<div class="content">
	...
	<ul id="top_topic_ul">
		<%
		posts = PostManager.getPublishAuthorPostsByScore(0, 6,author_id);
			    	it = posts.iterator();
			    	p = null;
		for(i=0; i<6; i++) {
			if(it.hasNext()){
    ...
    <div class="posts">
    	<%
    		if(page_num != 0) {
    	    	    	    		posts = PostManager.getPublishAuthorPosts(post_info_num*(page_num-1), post_info_num,author_id);
    	    	    	    	}else {
    	    	    	    		posts = PostManager.getPublishAuthorPosts(0,post_info_num,author_id);
    	    	    	    	}
    	    	    	    	it = posts.iterator();
    	    	    	    	p = null;
